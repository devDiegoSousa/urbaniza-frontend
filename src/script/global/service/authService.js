...
const authService = {
  ...
  async refreshAccessToken() {
    ...
    if (!refreshToken) {
      console.error("Erro: Refresh token não encontrado. Por favor, faça login novamente.");
    ...
    try {
      ...
      const errorData = await response.json();
        console.error("Erro ao renovar token da API:", errorData.message || response.statusText);
        throw new Error(`Erro ao renovar token: ${errorData.message || response.statusText}`);
  ...
  async authenticatedFetch(url, options = {}) {
      ...
      if (!accessToken) {
        console.error("Erro: Access token não encontrado. Faça login para acessar.");
      ...
      try {
        ...
        if (response.status === 401 && !options._retryCount) {
          ...
          try {
              ...
              console.error("Falha ao renovar token, redirecionando para login:", refreshError);
              throw refreshError; 
          }
        }
        ...
        if (response.status === 401 || response.status === 403) {
             this.clearAuthTokens();
             const errorData = await response.json();
             throw new Error(`Acesso negado: ${errorData.message || response.statusText}`);
        }
        return response;
      } catch (error) {
